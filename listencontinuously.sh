#!/bin/bash
ESPEAKPARAMS="-g5 -v en+f5 -s90"
LOGFILE=~/listenon28386.log
while true; do
  (~/bin/listenon28386.sh | tee -a $LOGFILE)
  echo espeak $ESPEAKPARAMS "incoming SMS message: $(tail -n1 $LOGFILE | jq '.data.payload.text')"
  espeak $ESPEAKPARAMS "incoming SMS message: $(tail -n1 $LOGFILE | jq '.data.payload.text')" -w /tmp/espeak.wav
